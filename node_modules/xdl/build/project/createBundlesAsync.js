...
function printBundleSizes(bundles) {
    ...
    if (bundles.ios.hermesSourcemap) {
        files.push([chalk_1.default.dim('index.ios.js.map (Hermes)'), bundles.ios.hermesSourcemap]);
    }
    else if (bundles.ios.map) {
        files.push([chalk_1.default.dim('index.ios.js.map'), bundles.ios.map]);
    }
    if (bundles.android.hermesSourcemap) {
        files.push([chalk_1.default.dim('index.android.js.map (Hermes)'), bundles.android.hermesSourcemap]);
    }
...
async function createBundlesAsync(projectRoot, publishOptions = {}, bundleOptions) {
    ...
    const isLegacy = config_1.isLegacyImportsEnabled(config.exp);
    const platforms = ['android', 'ios'];
    const [android, ios] = await dev_server_1.bundleAsync(projectRoot, config.exp, {
